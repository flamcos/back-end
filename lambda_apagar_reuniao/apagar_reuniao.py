...
def lambda_handler(event, context):
    ...
    # Espera receber o id da reunião a ser apagada no pathParameters
    try:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("DELETE FROM reunioes WHERE id = %s", (reuniao_id,))
            conn.commit()
            if cur.rowcount == 0:
                return {
                    'statusCode': 404,
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({'status': 'success', 'message': 'Reunião apagada com sucesso'})
    }