...
def lambda_handler(event, context):
    ...
    # Espera receber o id da reunião em pathParameters
    try:
        reuniao_id = int(event['pathParameters']['id'])
    except (KeyError, ValueError):
        return {
            'statusCode': 400,
            'body': json.dumps({'status': 'error', 'message': 'ID da reunião inválido'})
        }
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Busca avaliações
            cur.execute("""
                SELECT id, nota, descricao, email, data_hora_cadastro
                FROM avaliacoes
                WHERE reuniao_id = %s
            ...
            avaliacoes = [
                {
                    ...
                    'descricao': row[2],
                    'email': row[3],
                    'data_hora_cadastro': row[4].strftime('%Y-%m-%d %H:%M:%S')
                }
                for row in rows
            ]
    except Exception as e:
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({'status': 'success', 'avaliacoes': avaliacoes})
    }