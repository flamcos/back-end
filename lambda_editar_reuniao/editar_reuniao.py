...
def lambda_handler(event, context):
    ...
    # Espera receber o id da reunião em pathParameters e os dados no body
    try:
        reuniao_id = int(event['pathParameters']['id'])
    except (KeyError, ValueError):
        return {
            'statusCode': 400,
            'body': json.dumps({'status': 'error', 'message': 'ID inválido'})
        }
    ...
    try:
        ...
        participantes = body['lista_participante']
    except Exception:
        return {
            'statusCode': 400,
            'body': json.dumps({'status': 'error', 'message': 'Dados inválidos'})
        }
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE reunioes SET nome=%s, data_hora=%s, participantes=%s WHERE id=%s",
                (nome, data_hora, participantes, reuniao_id)
            )
            conn.commit()
            if cur.rowcount == 0:
                return {
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({'status': 'success', 'message': 'Reunião atualizada com sucesso'})
    }